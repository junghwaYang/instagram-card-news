...
13개 슬라이드 타입을 모두 포함하는 샘플 데이터(한국어, 디지털 마케팅 주제)를 내장하고, `sample-output/clean/` 디렉토리에 렌더링.

---

## 3단계: HTML 템플릿 생성 (clean 스타일 × 13타입 = 13개 파일)

**팀 에이전트를 사용해서 13개 타입을 병렬로 생성해줘.**
...
> 13종 슬라이드 타입 + clean 템플릿 스타일 + 팀 토론 파이프라인
...
**문제 발견 시**: Step 3으로 돌아가 구체적인 피드백과 함께 수정 요청
...
5. **clean 스타일 13개 HTML 템플릿 생성 (팀 에이전트 활용)**
6. CLAUDE.md 생성
7. npm install
8. 샘플 렌더링으로 검증
9. 에러 수정 (있다면)
